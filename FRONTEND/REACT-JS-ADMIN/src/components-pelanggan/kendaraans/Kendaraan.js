...
function DataKendaraan() {
	...
	const fetchData = async () => {
		...
		try {
			const response = await axios.get(
				"http://localhost:5162/GetKendaraanByIdUser/" + id_user
			);
	...
	const columns = [
		...
		},
		{
			name: "Ubah",
			selector: (row) => (
				<Link to={"/dataKendaraanPelanggan_edit/" + row.id_kendaraan} className="btn btn-primary">
					Edit
				</Link>
			),
			sortable: true,
		},
		{
			name: "Hapus",
			selector: (row) => (
				<Link to={"/dataKendaraanPelanggan_delete/" + row.id_kendaraan} className="btn btn-danger">
					Delete
				</Link>
			),
			sortable: true,
		...
		<div className="card" style={{ marginTop:150 }}>
			<div className="container">
				<div className="content">
					...
					<Link to="/dataKendaraanPelanggan_add" className="btn btn-primary">+ Tambah Kendaraan</Link>
